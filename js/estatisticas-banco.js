...
/* ════════════════════════════════════════════════════════════════════
 * CSS FIX — .qc-result.show position override
 * Garante que o painel de feedback (acerto/erro) apareça em fluxo normal
 * abaixo das alternativas, sem sobrepor o card via position:absolute.
 * ════════════════════════════════════════════════════════════════════ */
(function _injectQcResultFix(){
  if(document.getElementById('p01-qc-result-fix')) return;
  var s=document.createElement('style');
  s.id='p01-qc-result-fix';
  s.textContent=[
    /* Garante que o result-box do player foco/caderno fique em fluxo normal */
    '#resolucao-result-box.qc-result.show{',
    ...
    '}',
    '#resolucao-result-box.qc-result.acerto.show{',
      'background:rgba(74,222,128,.06);',
      'border-color:rgba(74,222,128,.25);',
    '}',
    '#resolucao-result-box.qc-result.erro.show{',
      'background:rgba(248,113,113,.06);',
      'border-color:rgba(248,113,113,.25);',
    '}',
    /* Garante que resultEl no modo lista também fique em fluxo */
    '[id^="resol-lista-result-"].qc-result.show{',
      'position:static!important;',
      'display:block!important;',
      'margin-top:.5rem;',
      'border-radius:10px;',
      'padding:.65rem .85rem;',
    '}',
    '[id^="resol-lista-result-"].qc-result.acerto.show{',
      'background:rgba(74,222,128,.06);',
      'border:1.5px solid rgba(74,222,128,.25);',
    '}',
    '[id^="resol-lista-result-"].qc-result.erro.show{',
      'background:rgba(248,113,113,.06);',
      'border:1.5px solid rgba(248,113,113,.25);',
    '}',
    /* qc-result oculto por padrão */
    '.qc-result:not(.show){display:none!important;}',
  ].join('');
...
// FIX: usar texto real das alternativas (q.questoes) e respeitar ABCD vs ABCDE
const letrasLista=tipo==='ABCD'?['A','B','C','D']:['A','B','C','D','E'];
const qOptsLista=q.questoes||{};
letrasLista.forEach(l=>{
const txtLista=qOptsLista[l]?renderTexto(qOptsLista[l]):'Alternativa '+l;
optsHTML+=`<div class="qc-opcao" id="opt-lista-${i}-${l}" onclick="selecionarResolListaResp('${l}',${i})">
<div class="qc-opcao-circulo">${l}</div><div class="qc-opcao-texto" style="text-align:left">${txtLista}</div></div>`;
});
}
div.innerHTML=`
<div class="qc-card">
...
${q.anotacao?`<details><summary style="font-size:.68rem;color:var(--blue);cursor:pointer;list-style:none">▶ Ver gabarito comentado</summary><div class="qc-result-gab" style="margin-top:.4rem;padding-top:.4rem;border-top:1px solid rgba(248,113,113,.2)">${renderTexto(q.anotacao)}</div></details>`:''}`;
  ...
  if(q._resposta){
    ...
    const keys2=tipo2==='CE'?['C','E']:tipo2==='ABCD'?['A','B','C','D']:['A','B','C','D','E'];
...
const keys=tipo==='CE'?['C','E']:tipo==='ABCD'?['A','B','C','D']:['A','B','C','D','E'];
opcoes.forEach((el,i)=>{
...
if(q._confirmado){
  ...
  if(q._acertou===true){
    ...
    resultBox.style.cssText='position:static;display:block;margin-top:.75rem';
