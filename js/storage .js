...
function _fbPushToCloud() {
  ...
  try {
    ...
    // Coleta todas as chaves eq2_ do localStorage (Desempenho Externo)
    const _eq2Data = {};
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k && k.startsWith('eq2_')) {
          _eq2Data[k] = JSON.parse(localStorage.getItem(k) || '{}');
        }
      }
    } catch(e) {}
...
function _fbPullFromCloud() {
  ...
  base.doc('shared').get().then(function(doc) {
    ...
    // Restaura dados do Desempenho Externo (eq2_*) do Firebase
    if (d.eq2Data && typeof d.eq2Data === 'object') {
      try {
        Object.keys(d.eq2Data).forEach(function(k) {
          if (k.startsWith('eq2_')) {
            const localData = JSON.parse(localStorage.getItem(k) || '{}');
            const cloudData = d.eq2Data[k] || {};
            // Usa nuvem se tiver mais tópicos preenchidos
            const localCount = Object.keys(localData).filter(function(t){ return localData[t] && localData[t].feitas > 0; }).length;
            const cloudCount = Object.keys(cloudData).filter(function(t){ return cloudData[t] && cloudData[t].feitas > 0; }).length;
            if (cloudCount > localCount) {
              localStorage.setItem(k, JSON.stringify(cloudData));
              console.log('[P01-FB] ✓ Desempenho Externo restaurado da nuvem para:', k);
            }
          }
        });
        // Notifica o módulo eq-v2 para re-renderizar se estiver ativo
        if (typeof window.eq2Build === 'function') {
          setTimeout(function() { try { window.eq2Build(); } catch(e) {} }, 500);
        }
      } catch(e) { console.warn('[P01-FB] eq2Data restore error:', e.message); }
    }
...
/* ════════════════════════════════════════════════════════════════════
 * _fbPushEq2ToCloud — sincroniza dados do Desempenho Externo (eq2_*)
 * Chamado pelo desempenho-externo.js após cada saveStats
 * ════════════════════════════════════════════════════════════════════ */
window._fbPushEq2ToCloud = function(key, data) {
  if (!_fbReady || !_fbDb || !_fbUserId) return;
  try {
    const eq2Payload = {};
    eq2Payload[key] = data;
    // Coleta todas as chaves eq2_ para envio completo
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith('eq2_') && k !== key) {
        try { eq2Payload[k] = JSON.parse(localStorage.getItem(k) || '{}'); } catch(e) {}
      }
    }
    _fbDb.collection('users').doc(_fbUserId).collection('state').doc('shared')
      .set({ eq2Data: eq2Payload, _ts: Date.now() }, { merge: true })
      .catch(function(e){ console.warn('[P01-FB] eq2 sync error:', e.message); });
  } catch(e) {
    console.warn('[P01-FB] _fbPushEq2ToCloud error:', e.message);
  }
};
...
`;
  ...
  window.p01FbGetAuthUser = function() { return _authUser; };
  window.p01FbOpenModal   = _openModal;
  ...
  function _startObserver() {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      _authUser = user;
      _updateBtn(user);
      if(typeof window.ppRefreshCloudStatus === 'function') window.ppRefreshCloudStatus();
      if (user && !user.isAnonymous) {
