...
function topbarUpdate(tabId, subTabId) {
  ...
  // Prioridade: ST.userProfile → p01_perfil_v1 → padrão
  let name  = (typeof ST !== 'undefined' && ST?.userProfile?.displayName) || '';
  let email = (typeof ST !== 'undefined' && ST?.userProfile?.email)       || '';
  if (!name || !email) {
    try {
      const saved = JSON.parse(localStorage.getItem('p01_perfil_v1') || '{}');
      if (!name  && saved.nome)  name  = saved.nome;
      if (!email && saved.email) email = saved.email;
    } catch(e) {}
  }
  name  = name  || 'Estudante';
  ...
  if (av) {
    try {
      const saved = JSON.parse(localStorage.getItem('p01_perfil_v1') || '{}');
      if (saved.foto) {
        av.innerHTML = '<img src="'+saved.foto+'" style="width:100%;height:100%;border-radius:50%;object-fit:cover">';
      } else {
        av.textContent = initials;
      }
    } catch(e) { av.textContent = initials; }
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Init imediato
  topbarSyncTheme();
  // Aplica perfil salvo imediatamente (evita piscar com valor padrão)
  try {
    const saved = JSON.parse(localStorage.getItem('p01_perfil_v1') || '{}');
    const nm = document.getElementById('topbar-user-name');
    const em = document.getElementById('topbar-user-email');
    const av2 = document.getElementById('topbar-avatar');
    if (nm && saved.nome)  nm.textContent = saved.nome;
    if (em && saved.email) em.textContent = saved.email;
    if (av2) {
      if (saved.foto) {
        av2.innerHTML = '<img src="'+saved.foto+'" style="width:100%;height:100%;border-radius:50%;object-fit:cover">';
      } else if (saved.nome) {
        const pts = saved.nome.trim().split(/\s+/);
        av2.textContent = (pts.length > 1 ? pts[0][0] + pts[pts.length-1][0] : saved.nome.slice(0,2)).toUpperCase();
      }
    }
  } catch(e) {}
