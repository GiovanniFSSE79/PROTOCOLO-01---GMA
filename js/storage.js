...
/* ════════════════════════════════════════════════════════════════════
 * P01 AUTH UI — Login com Google / Email por cima do Firebase existente
 * Aproveita o _fbDb e _fbConfig já inicializados neste arquivo.
 * ════════════════════════════════════════════════════════════════════ */
(function P01AuthUI() {
  'use strict';

  var _authUser = null; // usuário logado (null = anônimo)
...
.p01fb-btn-danger{background:rgba(248,113,113,.1);color:#f87171;
  border:1px solid rgba(248,113,113,.25);}
.p01fb-btn-info{background:rgba(96,165,250,.1);color:#60a5fa;
...
#p01-sync-bar{display:none;position:fixed;bottom:1rem;right:1rem;
  background:var(--surface);border:1px solid rgba(245,200,0,.3);
...
`;
  ...
  function _injectButton() {
    ...
    if (firstSep) {
      actions.insertBefore(btn, firstSep);
      actions.insertBefore(sep, btn);
    } else {
  ...
  /* ── Injeta modal e sync bar no DOM ── */
  ...
  function _renderModal() {
    ...
    if (_authUser && !_authUser.isAnonymous) {
      ...
      box.innerHTML =
        ...
        '<button class="p01fb-btn-primary p01fb-btn-info" onclick="p01FbRestaurar()">⬇ Restaurar da Nuvem</button>' +
  ...
  /* ── Aguarda Firebase estar pronto e pega instância de auth ── */
  function _getAuth(cb) {
  ...
  window.p01FbLoginGoogle = function() {
    _err(''); _ok('');
    _getAuth(function(auth) {
      var provider = new firebase.auth.GoogleAuthProvider();
      auth.signInWithPopup(provider).catch(function(e) { _err(_traduzir(e.code)); });
    });
  };
  ...
  window.p01FbLoginEmail = function() {
    ...
    _getAuth(function(auth) {
      auth.signInWithEmailAndPassword(email, senha).catch(function(e) { _err(_traduzir(e.code)); });
    });
  };
  ...
  window.p01FbTelaCadastro = function() {
    ...
    box.innerHTML =
      ...
      '<a class="p01fb-link" onclick="p01FbFecharModal();setTimeout(function(){p01FbFecharModal();},10)">← Voltar</a>';
  };
  ...
  window.p01FbSair = function() {
    _getAuth(function(auth) {
      auth.signOut().then(function() { _closeModal(); });
    });
  };
  ...
  window.p01FbRestaurar = function() {
    if (!_fbDb || !_fbUserId) { _err('Firebase não conectado.'); return; }
    _ok('Buscando backup...'); _err('');
    _fbPullFromCloud();
    setTimeout(function() { _ok('✅ Dados sincronizados! Recarregando...'); setTimeout(function(){ location.reload(); }, 1500); }, 2000);
  };

  /* ── Observer de autenticação — conecta ao Firebase existente ── */
  function _startAuthObserver() {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      ...
      if (user && !user.isAnonymous) {
        // Migra dados do uid anônimo para o uid real
        var oldUid = _fbUserId;
        _fbUserId = user.uid;
        localStorage.setItem('p01_fb_uid', user.uid);

        // Se trocou de uid, faz push imediato para salvar sob o novo uid
        if (oldUid !== user.uid) {
          setTimeout(_fbPushToCloud, 1000);
        }
  ...
  /* ── Helpers ── */
  function _err(msg) {
    var el = document.getElementById('p01fb-err');
    if (!el) return;
    el.textContent = msg;
    el.classList.toggle('show', !!msg);
  }
  function _ok(msg) {
    var el = document.getElementById('p01fb-ok');
    if (!el) return;
    el.textContent = msg;
    el.classList.toggle('show', !!msg);
  }
  ...
  function _init() {
    ...
    _injectModal();
    _injectButton();
    // Aguarda Firebase carregar (o _initFirebase roda no window load + 1s)
    var tries = 0;
    var check = setInterval(function() {
      ...
      if (tries > 30) clearInterval(check); // desiste após 15s
    }, 500);
  }
