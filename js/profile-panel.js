...
(function(){
  ...
  function _getProfile(){
    /* Fonte canônica: p01_perfil_v1 no localStorage.
       ST.userProfile é mantido em sincronia mas não é a fonte de verdade,
       pois o sistema multi-concurso não o persiste corretamente entre sessões. */
    var ls = _lsGet();
    var base = Object.assign({}, DEFAULTS, ls);
    // Mantém ST em sincronia para que outras partes do sistema possam ler
    if(typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = {};
      Object.assign(ST.userProfile, base);
    }
    return base;
  }

  function _saveProfile(){
    /* 1. Salva diretamente no localStorage (p01_perfil_v1) — sempre funciona */
    var p = (typeof ST !== 'undefined' && ST && ST.userProfile)
      ? ST.userProfile
      : {};
    _lsSet(p);
    /* 2. Tenta salvar via saveState() para manter Firebase em sincronia */
  ...
  function ppSaveIdentity(){
    ...
    /* Atualiza ST.userProfile */
    if(typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = Object.assign({},DEFAULTS);
      ST.userProfile.displayName = nome;
      ST.userProfile.email       = email;
    }
    /* Salva diretamente no localStorage (fonte canônica) */
    var ls = _lsGet();
    ls.displayName = nome;
    ls.email       = email;
    _lsSet(ls);
    /* Dispara saveState para Firebase */
    if(typeof saveState==='function') saveState();
    _showSaved();

    /* Atualiza UI da aba perfil */
    ...
    /* Atualiza topbar via p01SalvarPerfil (que tem o MutationObserver) */
    if(typeof window.p01SalvarPerfil === 'function'){
      window.p01SalvarPerfil({ nome: nome, email: email });
    } else if(typeof topbarUpdate==='function'){
      topbarUpdate('perfil','identidade');
    }
  }

  function ppResetIdentity(){
    if(typeof ST !== 'undefined' && ST && ST.userProfile){
      ST.userProfile.displayName = DEFAULTS.displayName;
      ST.userProfile.email       = DEFAULTS.email;
    }
    var ls = _lsGet();
    ls.displayName = DEFAULTS.displayName;
    ls.email       = DEFAULTS.email;
    _lsSet(ls);
    if(typeof saveState==='function') saveState();
    if(typeof window.p01SalvarPerfil==='function'){
      window.p01SalvarPerfil({ nome: DEFAULTS.displayName, email: DEFAULTS.email });
    }
   ...
   * ══════════════════════════════════════════════════════════════════ */
  function _bootstrap(){
    /* Lê do localStorage canônico e sincroniza com ST */
    var ls = _lsGet();
    if(typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = Object.assign({}, DEFAULTS);
      if(ls.displayName) ST.userProfile.displayName = ls.displayName;
      if(ls.email)       ST.userProfile.email       = ls.email;
      if(ls.theme)       ST.userProfile.theme       = ls.theme;
    }
    var p = _getProfile();
