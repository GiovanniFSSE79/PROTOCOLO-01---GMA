...
(function(){
  ...
  function buildProfile(){
    ...
    /* Injeta estilos do painel de nuvem se ainda não injetados */
    if(!document.getElementById('pp-cloud-styles')){
      var s=document.createElement('style');
      s.id='pp-cloud-styles';
      s.textContent=
        '.pp-cloud-user{display:flex;align-items:center;gap:.85rem;padding:.75rem;'+
        'background:rgba(74,222,128,.05);border:1px solid rgba(74,222,128,.18);border-radius:10px}'+
        '.pp-cloud-avatar{width:38px;height:38px;border-radius:50%;background:rgba(74,222,128,.15);'+
        'border:1.5px solid rgba(74,222,128,.3);display:flex;align-items:center;justify-content:center;'+
        'font-family:"Oswald",sans-serif;font-size:.95rem;font-weight:700;color:#4ade80;flex-shrink:0}'+
        '.pp-cloud-info{flex:1;min-width:0}'+
        '.pp-cloud-name{font-family:"Oswald",sans-serif;font-size:.82rem;font-weight:700;'+
  ...
  function _cloudSub(){
    ...
    var lastStr = lastTs ? new Date(parseInt(lastTs)).toLocaleString('pt-BR') : '—';
    return 'Último sync: '+lastStr;
  }
  function _renderCloudBody(){
    ...
    if(logado){
      ...
      '<div style="display:flex;gap:8px;flex-wrap:wrap">'+
        ...
        'onclick="p01FbBackupAgora();ppRefreshCloudStatus()" type="button">'+
      ...
      return '<div class="pp-connect" style="padding:1.2rem 0 .5rem">'+
        '<div class="pp-connect-icon" style="background:rgba(245,200,0,.08);border-color:rgba(245,200,0,.2)">'+
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="28" height="28"><path d="M4 20q0-4 8-4t8 4"/><circle cx="12" cy="8" r="4"/><path d="M18 8a6 6 0 0 1 0 12"/><path d="M21 5v6m-3-3h6" opacity=".6"/></svg>'+
        '</div>'+
  ...
  /* Chamado pelo storage.js quando o auth state muda */
