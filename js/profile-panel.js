...
(function(){
   ...
   * HELPERS GERAIS
   * ══════════════════════════════════════════════════════════════════ */
  var _LS_KEY = 'p01_perfil_v1';

  function _lsGet(){
    try{ return JSON.parse(localStorage.getItem(_LS_KEY)||'{}'); }catch(e){ return {}; }
  }
  function _lsSet(obj){
    try{ localStorage.setItem(_LS_KEY, JSON.stringify(obj)); }catch(e){}
  }

  function _getProfile(){
    /* Fonte canônica: p01_perfil_v1.
       index.html usa chave "nome"; profile-panel usa "displayName".
       Fazemos a ponte para que ambos sejam compatíveis. */
    var ls = _lsGet();
    var displayName = ls.displayName || ls.nome || DEFAULTS.displayName;
    var email       = ls.email || DEFAULTS.email;
    var base = Object.assign({}, DEFAULTS, ls, { displayName: displayName, email: email });
    if(typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = {};
      ST.userProfile.displayName = displayName;
      ST.userProfile.email       = email;
    }
    return base;
  }

  function _saveProfile(){
    if(typeof ST !== 'undefined' && ST && ST.userProfile){
      var p  = ST.userProfile;
      var ls = _lsGet();
      ls.nome        = p.displayName || ls.nome;
      ls.displayName = p.displayName || ls.displayName;
      ls.email       = p.email       || ls.email;
      _lsSet(ls);
      if(typeof window.p01SalvarPerfil === 'function'){
        window.p01SalvarPerfil({ nome: ls.nome, email: ls.email });
      }
    }
  ...
  /* ══════════════════════════════════════════════════════════════════
   * HELPERS — STATUS FIREBASE
   * ══════════════════════════════════════════════════════════════════ */
  function _fbUser(){
    return (typeof window.p01FbGetAuthUser === 'function') ? window.p01FbGetAuthUser() : null;
  }
  function _isLoggedIn(){
    var u = _fbUser(); return !!(u && !u.isAnonymous);
  }
  function _cloudBadge(){
    return _isLoggedIn()
      ? '<span class="pp-badge pp-badge-green">Conectado</span>'
      : '<span class="pp-badge pp-badge-dim">Não conectado</span>';
  }
  function _cloudSub(){
    if(!_isLoggedIn()) return 'Faça login para sincronizar';
    var lastTs = localStorage.getItem('p01_auth_last_backup');
    return lastTs ? 'Último sync: '+new Date(parseInt(lastTs)).toLocaleString('pt-BR') : 'Sincronizado';
  }
  function _renderCloudBody(){
    var u = _fbUser();
    var logado = _isLoggedIn();
    if(logado){
      var nome  = _esc(u.displayName || u.email || 'Usuário');
      var email = _esc(u.email || '');
      var lastTs  = localStorage.getItem('p01_auth_last_backup');
      var lastStr = lastTs ? new Date(parseInt(lastTs)).toLocaleString('pt-BR') : 'Nunca';
      return '<div class="pp-cloud-user">'+
        '<div class="pp-cloud-avatar">'+_esc((u.displayName||u.email||'U').charAt(0).toUpperCase())+'</div>'+
        '<div class="pp-cloud-info">'+
          '<div class="pp-cloud-name">'+nome+'</div>'+
          '<div class="pp-cloud-email">'+email+'</div>'+
        '</div>'+
        '<span class="pp-badge pp-badge-green" style="flex-shrink:0">☁ Ativo</span>'+
      '</div>'+
      '<div class="pp-bk-info-box" style="margin:1rem 0 .85rem">'+
        '✅ Backup automático: <strong style="color:var(--pp-green)">a cada 30 min</strong><br>'+
        '🕐 Último sync: <strong style="color:var(--pp-gold)">'+lastStr+'</strong>'+
      '</div>'+
      '<div style="display:flex;gap:8px;flex-wrap:wrap">'+
        '<button class="pp-btn pp-btn-primary" style="flex:1;justify-content:center" '+
        'onclick="p01FbBackupAgora();setTimeout(ppRefreshCloudStatus,2200)" type="button">'+
          _ico('cloud')+' Sincronizar agora'+
        '</button>'+
        '<button class="pp-btn pp-btn-ghost" style="color:#f87171;border-color:rgba(248,113,113,.3)" '+
        'onclick="p01FbSair()" type="button">Sair</button>'+
      '</div>';
    } else {
      return '<div class="pp-connect" style="padding:1rem 0 .5rem">'+
        '<div class="pp-connect-icon" style="background:rgba(245,200,0,.08);border-color:rgba(245,200,0,.2)">'+
          _ico('cloud')+
        '</div>'+
        '<div class="pp-connect-title">Entre para sincronizar</div>'+
        '<div class="pp-connect-desc">Seus dados ficam seguros na nuvem e acessíveis em qualquer dispositivo. Backup automático a cada 30 minutos.</div>'+
      '</div>'+
      '<div style="display:flex;flex-direction:column;gap:8px;margin-top:.25rem">'+
        '<button class="pp-btn" style="justify-content:center;gap:8px;background:rgba(255,255,255,.05);border:1px solid var(--pp-border-soft);color:var(--pp-text)" '+
        'onclick="p01FbOpenModal&&p01FbOpenModal()" type="button">'+
          ...
          'Entrar com Google'+
        '</button>'+
        '<button class="pp-btn pp-btn-secondary" style="justify-content:center" '+
        'onclick="p01FbOpenModal&&p01FbOpenModal()" type="button">Entrar com E-mail</button>'+
      '</div>';
    }
  }

  window.ppRefreshCloudStatus = function(){
    var badge = document.getElementById('pp-cloud-badge');
    var sub   = document.getElementById('pp-cloud-sub');
    var body  = document.getElementById('pp-cloud-body');
    if(badge) badge.innerHTML = _cloudBadge();
    if(sub)   sub.innerHTML   = _cloudSub();
    if(body)  body.innerHTML  = _renderCloudBody();
  };
  ...
  function _renderPanelDados(){
    ...
    return '<div class="pp-panel" id="pp-panel-dados">'+
      ...
      '<div class="pp-card">'+
        ...
        '<div class="pp-status-grid">'+
          ...
          _statusItem('cloud','cloud','Nuvem',
            '<span id="pp-cloud-badge">'+_cloudBadge()+'</span>',
            '<span id="pp-cloud-sub">'+_cloudSub()+'</span>')+
      ...
      '<div class="pp-card" id="pp-cloud-card">'+
        '<div class="pp-card-header">'+
          '<div class="pp-card-title">Sincronização na nuvem</div>'+
          '<div class="pp-card-desc">Faça login para sincronizar seus dados entre dispositivos automaticamente.</div>'+
        '</div>'+
        /* CSS inline para o user card logado */
        '<style>'+
          '.pp-cloud-user{display:flex;align-items:center;gap:.85rem;padding:.75rem;background:rgba(74,222,128,.05);border:1px solid rgba(74,222,128,.18);border-radius:10px}'+
          '.pp-cloud-avatar{width:38px;height:38px;border-radius:50%;background:rgba(74,222,128,.15);border:1.5px solid rgba(74,222,128,.3);display:flex;align-items:center;justify-content:center;font-family:"Oswald",sans-serif;font-size:.95rem;font-weight:700;color:#4ade80;flex-shrink:0}'+
          '.pp-cloud-info{flex:1;min-width:0}'+
          '.pp-cloud-name{font-family:"Oswald",sans-serif;font-size:.82rem;font-weight:700;color:var(--pp-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'+
          '.pp-cloud-email{font-size:.65rem;color:var(--pp-text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'+
        '</style>'+
        '<div id="pp-cloud-body">'+_renderCloudBody()+'</div>'+
      '</div>'+
  ...
  function ppSaveIdentity(){
    ...
    var nome  = (nm ? (nm.value||'').trim() : '') || DEFAULTS.displayName;
    var email = (em ? (em.value||'').trim() : '') || DEFAULTS.email;

    /* 1. Atualiza ST.userProfile */
    if(typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = Object.assign({},DEFAULTS);
      ST.userProfile.displayName = nome;
      ST.userProfile.email       = email;
    }
    /* 2. Salva com AMBAS as chaves no localStorage canônico */
    var ls = _lsGet();
    ls.nome        = nome;
    ls.displayName = nome;
    ls.email       = email;
    _lsSet(ls);
    /* 3. Notifica o sistema do index.html (topbar + MutationObserver) */
    if(typeof window.p01SalvarPerfil === 'function'){
      window.p01SalvarPerfil({ nome: nome, email: email });
    }
    /* 4. Sync Firebase */
    if(typeof saveState==='function') saveState();
    _showSaved();
    /* 5. Atualiza UI da aba */
    ...
    if(ab) ab.textContent=_initial(nome);
    if(nb) nb.textContent=nome;
    if(eb) eb.textContent=email;
  }
  ...
  function _bootstrap(){
    var ls = _lsGet();
    var nome = ls.displayName || ls.nome || '';
    if(nome && typeof ST !== 'undefined' && ST){
      if(!ST.userProfile) ST.userProfile = Object.assign({}, DEFAULTS);
      ST.userProfile.displayName = nome;
      if(ls.email) ST.userProfile.email = ls.email;
    }
    var p=_getProfile();
