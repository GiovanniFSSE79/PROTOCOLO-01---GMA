...
// CORREÇÃO: estas funções podem estar em scripts carregados APÓS app.js
// (eq-v2.js, simulados-v2.js, etc). Protegidas com typeof para evitar
// ReferenceError que trava toda a execução do arquivo antes de concInicializar().
try{ if(typeof populateSimAtivoSel==='function') populateSimAtivoSel(); }catch(e){}
try{ if(typeof populateSqmMatSel==='function')   populateSqmMatSel();   }catch(e){}
try{ if(typeof initBancoSelects==='function')    initBancoSelects();    }catch(e){}
try{ if(typeof initErrImportSelect==='function') initErrImportSelect(); }catch(e){}
try{ if(typeof bqRenderStats==='function')       bqRenderStats();       }catch(e){}
try{ if(typeof renderDashboard==='function')     renderDashboard(true); }catch(e){}
try{ if(typeof buildSessoesHistorico==='function') buildSessoesHistorico(); }catch(e){}
try{ if(typeof buildGraficoEvolucao==='function')  buildGraficoEvolucao();  }catch(e){}
...
// Coleta dados do Desempenho Externo (eq2_*) para incluir no backup
const _eq2Backup = {};
try {
  if (typeof window.eq2ExportStats === 'function') {
    Object.assign(_eq2Backup, window.eq2ExportStats());
  } else {
    for (let _i = 0; _i < localStorage.length; _i++) {
      const _k = localStorage.key(_i);
      if (_k && _k.startsWith('eq2_')) {
        try { _eq2Backup[_k] = JSON.parse(localStorage.getItem(_k) || '{}'); } catch(e) {}
      }
    }
  }
} catch(e) {}
...
// Desempenho Externo
desempenhoExterno:Object.keys(_eq2Backup).length,
}
},
...
// ── Desempenho Externo (eq2_*) — FASE 9.4.15 ──
eq2Data: _eq2Backup,
};
...
// FASE 9.4.15: restaurar Desempenho Externo (eq2_*)
const bkEq2Data = _bkDados.eq2Data || null;
if(bkEq2Data && typeof bkEq2Data === 'object'){
  try{
    Object.keys(bkEq2Data).forEach(function(k){
      if(k.startsWith('eq2_')){
        const bkTopics = bkEq2Data[k] || {};
        const localRaw = localStorage.getItem(k);
        const localTopics = localRaw ? JSON.parse(localRaw) : {};
        // Mescla: usa o maior valor entre backup e local para cada tópico
        const merged = Object.assign({}, bkTopics);
        Object.keys(localTopics).forEach(function(tid){
          if(!merged[tid] || (localTopics[tid].feitas||0) > (merged[tid].feitas||0)){
            merged[tid] = localTopics[tid];
          }
        });
        localStorage.setItem(k, JSON.stringify(merged));
      }
    });
    // Re-renderiza o módulo se estiver ativo
    if(typeof window.eq2Build === 'function') setTimeout(function(){ try{window.eq2Build();}catch(e){} }, 800);
    console.log('[P01 Backup] Desempenho Externo restaurado:', Object.keys(bkEq2Data).join(', '));
  }catch(e){ console.warn('[P01 Backup] Erro ao restaurar eq2Data:', e.message); }
}
